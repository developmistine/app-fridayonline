...
class ShopFlashSale extends StatefulWidget {
  const ShopFlashSale({super.key});

  @override
  State<ShopFlashSale> createState() => _FlashSaleEndUserState();
}

class _FlashSaleEndUserState extends State<ShopFlashSale> {
  ...
  void fetchMoreProductContent() async {
    brandCtr.isLoadMoreFlashSale.value = true;

    try {
      // ดึงข้อมูลใหม่จาก API
      final newFlashSale = await brandCtr.fetchMoreFlashSale(offset);

      if (newFlashSale!.data.productContent.isNotEmpty) {
        brandCtr.shopFlashSale!.data.productContent
            .addAll(newFlashSale.data.productContent);
        offset += 20;
      }
    } finally {
      brandCtr.isLoadMoreFlashSale.value = false;
    }
  }
                        ...
                        return InkWell(
                          onTap: () {
                            Get.find<ShowProductSkuCtr>().fetchB2cProductDetail(
                                product.productId, 'shop_flashsale');
                            Get.toNamed(
  ...
  double percentProgressColor(ProductContent product) {
    double percent = product.stock / product.flashSaleStock;
    bool isLess = percent < 0;
    bool isMore = percent > 1;
    if (isLess) {
      return 0.0;
    } else if (isMore) {
      return 1.0;

import 'dart:convert';
import 'package:fridayonline/model/cart/cart_popup_nocat.dart';
import 'package:fridayonline/service/pathapi.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

import '../../../model/set_data/set_data.dart';

Future<PopupCartNocat?> getCartPopupNocat() async {
  SetData data = SetData();
  try {
    var url = Uri.parse("${base_api_app}api/v1/cart/GetCartPopup");
    var jsonInsert = jsonEncode({
      "RepSeq": await data.repSeq,
      "RepCode": await data.repCode,
      "RepType": await data.repType,
      "UserId": await data.enduserId,
      "Device": await data.device,
    });
    var res = await http.post(url,
        headers: <String, String>{
          'Authorization': 'Bearer $apiToken',
          'Content-type': 'application/json; charset=utf-8'
        },
        body: jsonInsert);
    if (res.statusCode == 200) {
      final popupCartNocat = popupCartNocatFromJson(res.body);
      return popupCartNocat;
    }
  } catch (e) {
    debugPrint("erroor popup cheer is $e");
  }
  return null;
}

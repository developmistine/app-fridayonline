...
class CheckMslOtpWithPhone extends StatefulWidget {
  final String repcode;
  final String reptel;
  const CheckMslOtpWithPhone(
      {super.key, required this.repcode, required this.reptel});

  @override
  State<CheckMslOtpWithPhone> createState() => _CheckMslOtpState();
}

class _CheckMslOtpState extends State<CheckMslOtpWithPhone> {
  ...
  void initState() {
    ...
    lmphonenumber.text = widget.reptel;
    super.initState();
  }
                            ...
                            onPressed: () async {
                              ...
                              if (validate) {
                                if (!timerController.isCountDown.value) {
                                  Otpenduserobject res =
                                      await postotpENDUSER(lmphonenumber.text);
                                  // printWhite(res.toJson());
                                  if (res.values == '001') {

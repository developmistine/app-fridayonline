...
class CheckInformationOrderAll extends StatefulWidget {
  const CheckInformationOrderAll({super.key});

  @override
  State<CheckInformationOrderAll> createState() =>
      _CheckInformationOrderAllState();
}

class _CheckInformationOrderAllState extends State<CheckInformationOrderAll> {
  final Future<SharedPreferences> _pref = SharedPreferences.getInstance();
  // ignore: non_constant_identifier_names
  bool approve_all = false;

  //ฟังก์ชันสำหรับแสดง alert เมื่อทำการแก้ไขรายการสำเร็จ
          ...
          child: CupertinoAlertDialog(
            ...
            actions: <Widget>[
              CupertinoDialogAction(
                ...
                child: Text('ตกลง', style: TextStyle(color: theme_color_df)),
              ),
            ],
          ),
        );
              ...
              child:
                ...
                if (dataResponse.isDataLoading.value) {
                  ...
                  return Column(
                    ...
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      if (dataResponse.response!.values == '1')
                        Center(
                          child: Lottie.asset(
                              width: 230,
                              height: 230,
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: call_information_order_order(
          '9'), //เรียกใช้งานฟังก์ชัน call_information_order_order เพื่อ get ข้อมูลหน้าปกแค็ตตาล็อก Ecatalog
      builder: (BuildContext context,
        ...
        if (snapshot.connectionState == ConnectionState.done) {
          ...
          if (result!.customerOrderHistory.customerMslOrderDetail.isNotEmpty ||
            ...
            var i = 0;
            while (i <
                result.customerOrderHistory.customerEndUserOrderDetail.length) {
              if (result.customerOrderHistory.customerEndUserOrderDetail[i]
                      .statusOrder ==
                  'N') {
              ...
              child: Column(
                children: [
                  ListView.builder(
                    ...
                    shrinkWrap: true,
                    itemCount: result
                        .customerOrderHistory.customerMslOrderDetail.length,
                  ...
                  // ? order ลูกค้า
                  result.customerOrderHistory.customerEndUserOrderDetail
                          .isNotEmpty
                      ? Padding(
                          padding:
                              const EdgeInsets.only(top: 0, left: 3, bottom: 0),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(
                                  MultiLanguages.of(context)!
                                      .translate('txt_customer_orders'),
                                  style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 15,
                                      color: theme_color_df)),
                              //ตรวจสอบว่ามีรายการสั่งซื้อสถานะ "ลูกค้ารออนุมัติ" ไหม
                              approve_all == true
                                  //กรณีที่มีรายการสั่งซื้อสถานะ "ลูกค้ารออนุมัติ" ให้แสดงปุ่ม "อนุมัติทั้งหมด"
                                  ? Row(
                                      // mainAxisAlignment: MainAxisAlignment.end,
                                      children: [
                                        TextButton(
                                          onPressed: () async {
                                            ...
                                            var data = ApproveEndUserAll(
                                                listUpdateAll: detail);
                                            var json =
                                                data.toJson(); //แปลงเป็น json
